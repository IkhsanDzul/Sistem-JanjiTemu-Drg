    ...
    <div class="bg-gradient-to-r from-[#005248] to-[#007a6a] rounded-lg shadow-md p-6 text-white">
        <div class="flex items-center justify-between">
            ...
            <div class="hidden md:flex items-center gap-2 bg-white/10 px-4 py-2 rounded-lg backdrop-blur-sm">
                ...
                <span class="text-sm font-medium">Total: {{ $appointments->total() }}</span>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white rounded-lg shadow-sm border border-gray-100 p-4">
        <div class="flex flex-col md:flex-row gap-4 items-center justify-between">
            ...
            <div class="flex gap-2 mb-4">
                ...
                {{ $filter == null ? 'bg-[#005248] text-white' : 'bg-gray-100 text-gray-700 hover:bg-[#FFA700] hover:text-white' }}">
                Semua
                </a>
             
                <a href="{{ route('dokter.janji-temu.index', ['filter' => 'today']) }}"
                class="filter-btn px-4 py-2 rounded-lg text-sm font-medium
                {{ $filter === 'today' ? 'bg-[#005248] text-white' : 'bg-gray-100 text-gray-700 hover:bg-[#FFA700] hover:text-white' }}">
                Hari Ini
                </a>
             
                <a href="{{ route('dokter.janji-temu.index', ['filter' => 'pending']) }}"
                class="filter-btn px-4 py-2 rounded-lg text-sm font-medium
                {{ $filter === 'pending' ? 'bg-[#005248] text-white' : 'bg-gray-100 text-gray-700 hover:bg-[#FFA700] hover:text-white' }}">
                Pending
                </a>
             
                <a href="{{ route('dokter.janji-temu.index', ['filter' => 'confirmed']) }}"
                class="filter-btn px-4 py-2 rounded-lg text-sm font-medium
                {{ $filter === 'confirmed' ? 'bg-[#005248] text-white' : 'bg-gray-100 text-gray-700 hover:bg-[#FFA700] hover:text-white' }}">
                Confirmed
                </a>
             
                <a href="{{ route('dokter.janji-temu.index', ['filter' => 'completed']) }}"
                class="filter-btn px-4 py-2 rounded-lg text-sm font-medium
                {{ $filter === 'completed' ? 'bg-[#005248] text-white' : 'bg-gray-100 text-gray-700 hover:bg-[#FFA700] hover:text-white' }}">
                Completed
                </a>
            </div>
        </div>
                ...
                <div class="flex items-start justify-between">
                    <div class="flex items-center gap-3">
                        <div class="w-12 h-12 rounded-full flex items-center justify-center text-white font-semibold text-lg shadow-md
                            ...
                            @endif">
                            {{ strtoupper(substr($item->pasien->user->nama_lengkap ?? 'P', 0, 1)) }}
                        </div>
                        <div>
                            <h3 class="font-bold text-gray-900 text-base">
                                {{ $item->pasien->user->nama_lengkap ?? 'Nama Tidak Ada' }}
                            </h3>                                
                            <p class="text-xs text-gray-500 mt-0.5">ID: #{{ $item->id }}</p>
                        </div>
                    </div>
    ...
    <div class="bg-white rounded-lg shadow-sm border border-gray-100 p-4">
        <div class="flex flex-col sm:flex-row items-center justify-between gap-4">
            ...
            <div class="text-sm text-gray-600">
                Menampilkan <span class="font-semibold text-gray-900">{{ $appointments->firstItem() }}</span> 
                sampai <span class="font-semibold text-gray-900">{{ $appointments->lastItem() }}</span> 
                dari <span class="font-semibold text-gray-900">{{ $appointments->total() }}</span> data
            </div>
            ...
            <div class="flex items-center gap-2">
                ...
                @else
                    <a href="{{ $appointments->appends(['filter' => $filter])->previousPageUrl() }}" 
                       class="px-3 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-lg hover:bg-[#005248] hover:text-white hover:border-[#005248] transition-colors">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"/>
                        </svg>
                    </a>
                ...
                @foreach ($appointments->getUrlRange(1, $appointments->lastPage()) as $page => $url)
                    @if ($page == $appointments->currentPage())
                        <span class="px-4 py-2 text-sm font-semibold text-white bg-[#005248] rounded-lg">
                            {{ $page }}
                        </span>
                    @else
                        <a href="{{ $appointments->appends(['filter' => $filter])->url($page) }}" 
                           class="px-4 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-lg hover:bg-[#005248] hover:text-white hover:border-[#005248] transition-colors">
                            {{ $page }}
                        </a>
                    @endif
                @endforeach
                ...
                @if ($appointments->hasMorePages())
                    <a href="{{ $appointments->appends(['filter' => $filter])->nextPageUrl() }}" 
                       class="px-3 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-lg hover:bg-[#005248] hover:text-white hover:border-[#005248] transition-colors">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
